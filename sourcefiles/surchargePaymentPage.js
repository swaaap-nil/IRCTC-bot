var arr = [],
    ...
    dcCopy =
        'Rupay Debit Card: Zero Charges. For non Rupay Debit Card: 0.4% of load upto Rs. 2000 and 0.9% of load above Rs. 2000.',
...
function deleteSaveCard(saveThisCard) {
    if (saveThisCard) {
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                var deleteThisSaveCard = document.getElementById('tokenId' + currentTokenIdSaveCard)
...
function handleClick(myRadio) {
    ...
    var selectedLi = document.getElementsByClassName('selectedSaveDetails')[0],
        paymentType = document
            .querySelectorAll('.selectedSaveDetails .text-muted')[0]
            .innerText.trim()
            .replace(/\s/g, ''),
        mopType = document
            .querySelectorAll('.selectedSaveDetails .saveMobImg')[0]
            .getAttribute('alt')
            .replace(/\s/g, '')
            .toUpperCase()
    if (paymentType == 'CreditCard') {
        ...
        if (merchantType == 'IRCTC iMudra') {
            document.getElementById('orderfootDetails1').innerHTML =
                'Credit Card: 1% of load amount.'
            document.getElementById('orderfootDetails2').innerHTML =
                'Credit Card: 1% of load amount.'
        } else {
...
function saveCardPay() {
    ...
    var selectedSaveDetailsCvvVal = document.querySelectorAll(
        '.selectedSaveDetails .savDetailsCvv',
    )[0].value
    ...
    if (selectedSaveDetailsCvvVal) {
        if (selectedSaveDetailsCvvVal.length < 3) {
            ...
            var currentTokenId = document.querySelectorAll('.selectedSaveDetails .visaRadio')[0]
                .value
            var saveMobImg = document
                .querySelectorAll('.selectedSaveDetails .saveMobImg')[0]
                .getAttribute('alt')
                .toUpperCase()
            var savedCardNumber = document.querySelectorAll('.selectedSaveDetails .saveCardNum')[0]
                .value
...
function showStuff(event, currentElement, paymentType) {
    ...
    switch (paymentType) {
        case 'saveCard':
            var checkPaymentTypeInSaveCard = document
                .querySelector('.selectedSaveDetails .text-muted')
                .innerText.trim()
                .replace(/\s/g, '')
                .toLowerCase()
            merchantType = pageInfoObj.merchantType
            if (checkPaymentTypeInSaveCard == 'creditcard') {
                ...
                if (merchantType == 'IRCTC iMudra') {
                    orderfootDetails1.innerHTML =
                        'Rupay Debit Card: Zero Charges. For non Rupay Debit Card: 0.4% of load upto Rs. 2000 and 0.9% of load above Rs. 2000.'
                    orderfootDetails2.innerHTML =
                        'Rupay Debit Card: Zero Charges. For non Rupay Debit Card: 0.4% of load upto Rs. 2000 and 0.9% of load above Rs. 2000.'
                } else {
...
function pageInfo() {
    ...
    for (var nbMop = 0; nbMop < obj.nbMopType.length; nbMop++) {
        ...
        document.getElementById('netbankingList').innerHTML =
            '<datalist id="banks">' + banksOption + '</datatlist>'
        ...
        if (collectNetbankingMob == 'State Bank Of India') {
            if (checkBankList('StateBankOfIndia')) {
                li.innerHTML =
                    "<li><img src='../image/sbi.png' id='StateBankOfIndia' alt='SBI' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'Axis Bank') {
            if (checkBankList('AxisBank')) {
                li.innerHTML =
                    "<li><img src='../image/axis.png'  id='AxisBank' alt='AXIS' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'HDFC Bank') {
            if (checkBankList('HDFCBank')) {
                li.innerHTML =
                    "<li><img src='../image/hdfc.png' id='HDFCBank' alt='HDFC' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'ICICI Bank') {
            if (checkBankList('ICICIBank')) {
                li.innerHTML =
                    "<li><img src='../image/icici.png' id='ICICIBank' alt='ICICI' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'Punjab National Bank') {
            if (checkBankList('PunjabNationalBank')) {
                li.innerHTML =
                    "<li><img src='../image/pnb.png' id='PunjabNationalBank' alt='PNB' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'Kotak Bank') {
            if (checkBankList('KotakBank')) {
                li.innerHTML =
                    "<li><img src='../image/kotak.png' id='KotakBank' alt='Kotak' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        } else if (collectNetbankingMob == 'Bank of Baroda') {
            if (checkBankList('BankofBaroda')) {
                li.innerHTML =
                    "<li><img src='../image/bob.png' id='BankofBaroda' alt='BOB' onClick='bankImageClicked(this.id)'/></li>"
                firstList.appendChild(li)
            }
        }
    ...
    for (var i = 0; i < cardLength; i++) {
        if (obj.payMode == 'ALL') {
            if (
                pageInfoObj.express_pay &&
                document.getElementsByClassName('saveCardDetails').length &&
                cardArray[i] == 'sc'
            ) {
                allLi +=
                    '<li class="tabLi" id="saveCardLi" onmousedown="showStuff(event, this, ' +
                    "'saveCard'" +
                    ')" data-id="saveCard">Saved Cards</li>'
            }

            if (obj.creditCard && cardArray[i] == 'cc') {
                var creditCard = 'creditCard'
                allLi +=
                    '<li class="tabLi" id="creditLi" onmousedown="showStuff(event, this, ' +
                    "'creditCard'" +
                    ')" data-id="debitWithPin">Credit Card</li>'
            ...
            if (obj.debitCard && cardArray[i] == 'dc') {
                ...
                allLi +=
                    '<li class="tabLi" id="debitLi" onmousedown="showStuff(event, this, ' +
                    "'debitCard'" +
                    ')" data-id="debitWithPin">Debit Card</li>'
            ...
            if (obj.prepaidCard && cardArray[i] == 'pc') {
                ...
                allLi +=
                    '<li class="tabLi" id="prepaidLi" onmousedown="showStuff(event, this, ' +
                    "'prepaidCard'" +
                    ')" data-id="debitWithPin">Prepaid Card</li>'
            ...
            if (obj.autoDebit && cardArray[i] == 'autoDebit') {
                if (obj.autope) {
                    var mandateDiv = ''
                    allLi +=
                        '<li class="tabLi" id="autoDebitLi" onmousedown="showStuff(event, this,' +
                        "'autoDebit'" +
                        ')" data-id="autoDebit">autopay</li>'
            ...
            if (obj.iMudra && cardArray[i] == 'iMudra') {
                allLi +=
                    '<li class="tabLi" id="iMudraLi" onmousedown="showStuff(event, this,' +
                    "'iMudra'" +
                    ')" data-id="iMudra">irctc imudra</li>'
            }

            if (obj.netBanking && cardArray[i] == 'netBanking') {
                allLi +=
                    '<li class="tabLi" id="netBankingLi" onmousedown="showStuff(event, this,' +
                    "'netBanking'" +
                    ')" data-id="netBanking">net Banking</li>'
            }

            if (obj.upi && cardArray[i] == 'upi') {
                allLi +=
                    '<li class="tabLi" id="upiLi" onmousedown="showStuff(event, this, ' +
                    "'upi'" +
                    ')" data-id="upi">UPI</li>'
            }
            ...
            if (showVouchaGram == 'Y') {
                if (cardArray[i] == 'giftVoucher_wrapper') {
                    if (obj.autope) {
                        var mandateDiv = ''
                        allLi +=
                            '<li class="tabLi" id="gyftrLi" onclick="showStuff(event, this,' +
                            "'autoDebit'" +
                            '); gyftrFunction();" data-id="autoDebit" >Gift Voucher</li>'
                    }
                }
            }
            ...
            if (showVouchaGram == 'Y') {
                if (cardArray[i] == 'giftVoucher_wrapper') {
                    if (obj.autope) {
                        var mandateDiv = ''
                        allLi +=
                            '<li class="tabLi" id="gyftrLi" onclick="gyftrFunction();" onmousedown="showStuff(event, this,' +
                            "'autoDebit'" +
                            ')" data-id="autoDebit" >Gift Voucher</li>'
                    }
                }
            }
            ...
            if (obj.upi && obj.googlePay && cardArray[i] == 'gpay') {
                allLi +=
                    '<li class="tabLi" id="gpayLi" onmousedown="showStuff(event, this, ' +
                    "'googlePay'" +
                    ')" data-id="googlePay"><img src="../image/gpay.png"/></li>'
            }
...
function addConvenienceFee(paymentType, moptype) {
    ...
    switch (paymentType) {
        case 'creditCard':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_cc) + Number(innerAmount)).toFixed(2)
            document.getElementById('confirm-purchase').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_cc) + Number(innerAmount)).toFixed(2)
            document.getElementById('exSubmit').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_cc) + Number(innerAmount)).toFixed(2)
        ...
        case 'debitCard':
            if (moptype == 'RU') {
                ...
                totalAmount.textContent =
                    currencyCode + ' ' + (0.0 + Number(innerAmount)).toFixed(2)
                document.getElementById('confirm-purchase').value =
                    'Pay ' + currencyCode + ' ' + (0.0 + Number(innerAmount)).toFixed(2)
                document.getElementById('exSubmit').value =
                    'Pay ' + currencyCode + ' ' + (0.0 + Number(innerAmount)).toFixed(2)
            } else {
                ...
                totalAmount.textContent =
                    currencyCode +
                    ' ' +
                    (Number(pageInfoObj.surcharge_dc) + Number(innerAmount)).toFixed(2)
                document.getElementById('confirm-purchase').value =
                    'Pay ' +
                    currencyCode +
                    ' ' +
        ...
        case 'prepaidCard':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_pc) + Number(innerAmount)).toFixed(2)
            document.getElementById('confirm-purchase').value =
                'Pay ' +
                currencyCode +
                ' ' +
        ...
        case 'upi':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_up) + Number(innerAmount)).toFixed(2)
            document.getElementById('upi-sbmt').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_up) + Number(innerAmount)).toFixed(2)
        ...
        case 'googlePay':
            ...
            totalAmount.textContent =
                ...
                ' ' +
                (Number(pageInfoObj.surcharge_up) + Number(innerAmount)).toFixed(2)
            document.getElementById('googlePayBtn').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_up) + Number(innerAmount)).toFixed(2)
        ...
        case 'autoDebit':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_ad) + Number(innerAmount)).toFixed(2)
            autoDebitBtnVal =
                'Pay ' +
                currencyCode +
                ' ' +
        ...
        case 'iMudra':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_wl) + Number(innerAmount)).toFixed(2)
            document.getElementById('iMudraBtn').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_wl) + Number(innerAmount)).toFixed(2)
        ...
        case 'netBanking':
            ...
            totalAmount.textContent =
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_nb) + Number(innerAmount)).toFixed(2)
            document.getElementById('netBankingBtn').value =
                'Pay ' +
                currencyCode +
                ' ' +
                (Number(pageInfoObj.surcharge_nb) + Number(innerAmount)).toFixed(2)
    ...
    document.getElementById('amountPayablePhone').innerHTML =
...
function binCheck(event) {
    ...
    xhr.onload = function () {
        ...
        if (
            ...
            obj.paymentType == getPaymentTypeFromUi
        ) {
            document.getElementById('notSupportedCard').style.display = 'none'
            ...
            if (document.getElementById(obj.mopType.toLowerCase() + 'cc') != null) {
                document
                    .getElementById(obj.mopType.toLowerCase() + 'cc')
                    .classList.remove('opacityMob')
            ...
            if (document.getElementById(obj.mopType.toLowerCase() + 'dc') != null) {
                document
                    .getElementById(obj.mopType.toLowerCase() + 'dc')
                    .classList.remove('opacityMob')
            ...
            if (document.getElementById(obj.mopType.toLowerCase() + 'pc') != null) {
                document
                    .getElementById(obj.mopType.toLowerCase() + 'pc')
                    .classList.remove('opacityMob')
    ...
    if (ele.value.length == 11) {
        ...
        xhr.onload = function () {
            ...
            if (obj.mopType && obj.issuerCountry != null && obj.issuerBankName != null) {
                mopType = obj.mopType
                let mandateSelected = document.querySelector(
                    'input[name="mandateType"]:checked',
                ).value
                if (
                    (mandateSelected == 'ccMandate' && obj.paymentType == 'CC') ||
                    (mandateSelected == 'accountMandate' &&
                        (obj.paymentType == 'DC' || obj.paymentType == 'CC'))
                ) {
                    ...
                    switch (paymentType.toUpperCase()) {
                        ...
                        case 'DC':
                            radioButton = document.querySelector(
                                '.radioWrapper .accountMandate input',
                            )
                            isAnyPaymentType = true
                            break
            ...
            if (
                obj.isYesBankBin == 'Y' &&
                VOUCHAGRAM_USED == false &&
                ADVANTAGECLUB_USED == false
            ) {
...
function enterCardNum(event) {
    ...
    if (alreadyPopulated) {
        decideBinCheck(
            document.querySelector('.cardNumber').value.replace(/\s/g, '').substring(0, 9),
            event,
        )
    }
    ...
    if (
        ...
        checkCvv() &&
        nameCheckKeyUp() &&
        checkMopTypeValidForUser() &&
        checkLuhn(cardNumberElement)
    ) {
        document.getElementById('confirm-purchase').classList.add('actvBtnCreditDebit')
    } else {
        document.getElementById('confirm-purchase').classList.remove('actvBtnCreditDebit')
...
function checkFields(e, element) {
    ...
    document.getElementById('setExpiryMonth').value = document
        .getElementById('paymentDate')
        .value.split('/')[0]
    document.getElementById('setExpiryYear').value =
        '20' + document.getElementById('paymentDate').value.split('/')[1]
...
function autoDebitFn() {
    var innerAmountValue = document
        .getElementById('innerAmount')
        .innerHTML.replace('INR', '')
        .trim()
    ...
    document.querySelector('#payAutoDebit input[name="expiryMonth"]').value = document
        .getElementById('mandateDate')
        .value.split('/')[0]
    document.querySelector('#payAutoDebit input[name="expiryYear"]').value =
        '20' + document.getElementById('mandateDate').value.split('/')[1]
    ...
    for (const radio of radioBtns) {
        if (radio.checked) {
            ...
            switch (mandateType) {
                case 'upiMandate':
                    if (vpaValidation()) {
                        var vpa = document.querySelector(
                            '#upiMandate_wrapper input[name="VPA"]',
                        ).value
                ...
                case 'accountMandate':
                    ...
                    if (yesBankRewardPoints == null || yesBankRewardPoints == '') {
                        ...
                        var pointsPeMobileNumberEle =
                            document.getElementById('pointsPeMobileNumber')
                        pointsPeMobileNumberEle.value = ''
                        document.getElementById('ppValueOfPointsDiv').style.visibility = 'hidden'
                        document.getElementById('pointsPeMobileNumberDiv').style.display = 'none'
                    }
                    ...
                    if (
                        pointsPeChkBx.checked &&
                        parseFloat(yesBankRewardPoints) >= parseFloat(orderTotalAmount)
                    ) {
                        ...
                        if (isCardNumberValid) {
                            ...
                            document.getElementById('preAuthOn').value =
                                payEle.getAttribute('ipayPreAuth')
...
function checkCardNumber(cardNo) {
    ...
    if (cardNo.value != '') {
        ...
        if (!result) {
            ...
            mndtCardNo.classList.add('redLine')
            return result
        } else document.getElementById('mndtInvalidCardNo').style.display = 'none'
    } else {
...
function mopTypeIconShow(getMopType) {
    ...
    if (getMopTypeLowerCase == 'bc') {
        if (document.getElementById('userMoptypeIcon').src.indexOf('bc.png') == -1) {
            document.getElementById('userMoptypeIcon').src =
                '../image/' + getMopTypeLowerCase + '.png'
        }
    } else {
...
function checkErrorMsgShowOrNot() {
    ...
    if (
        checkStartNoDisplay == 'block' ||
        validCardCheckDisplay == 'block' ||
        emptyCardNumberDisplay == 'block' ||
        notSupportedCardDisplay == 'block'
    ) {
...
function switch_radiobtn(el, e) {
    ...
    switch (boxClass) {
        case 'upiMandate':
            ...
            canPara.innerText = ''
            canPara.innerHTML +=
                '<label style="margin-left:10px"></label>* In case if the transaction fails, mandate will be released within 30 minutes of transaction else you may reach us on <a href="mailto: support@autope.in">support@autope.in</a>'
        ...
        case 'ccMandate':
            verifyCust()
            canPara.innerText =
                'Note : By selecting this option, the user is signing a mandate on Credit Card, subsequent transactions will be done without 2nd factor of authentication (OTP).'
        ...
        case 'vouchaGram':
        	...
        	*/
            ...
            canPara.innerText = ''
            canPara.innerHTML +=
                '<label style="margin-left:10px"></label>* In case if the transaction fails, Refund will be processed within 30 minutes of transaction else you may reach us on <a href="mailto: support@autope.in">support@autope.in</a>'
    ...
    verifyXhr.onload = function () {
        ...
        if (obj.preMandateStatus == 'success') {
            ...
            <div id="exist-mandate" class="p2">
                <p class="t-center">There is an existing mandate on your account. If you wish to continue on the same, then you will receive an OTP on the mobile number ` +
                obj.mobileNo +
                `. If you proceed with the existing mandate, then your transaction will go through without Second Factor Authentication.</p>


                <div class="justify mt-15">
                <button type="button" class="mandate-btn" onclick="executeExistingMandate();">Verify via<br/>(M)` +
                obj.mobileNo +
                `</button>
...
function executeSIDebitExistingMandate() {
    ...
    mandateXhr.onload = function () {
        ...
        for (key in verifyCustResponse) {
            res +=
                '<input type="hidden" name="' + key + '" value="' + verifyCustResponse[key] + '">'
        }
...
function giftVoucherValidation() {
    ...
    */
    ...
    if (vouchaGramMobileNumberVal.length == 10) {
        if (
            !/^[0-5]/.test(vouchaGramMobileNumberVal) &&
            isValidMobileNumber(vouchaGramMobileNumberVal)
        ) {
            vouchaGramMobileNumberEle.classList.remove('redLine')
            var vgMobNumMsg = document.getElementById('vgMobNumMsg')
            vgMobNumMsg.innerHTML = ''
            ...
            if (resData != null) {
                ...
                if (balance > 0) {
                    ...
                    if (voucherListArr != null && voucherListArr.length > 0) {
                        ...
                        vgMobNumMsg.style.display = 'block'
                        vgMobNumMsg.innerHTML =
                            'You do not have enough balance in your GyFTR wallet, please use another mobile number.'
                    }
                }
                ...
                if (voucherListArr != null && voucherListArr.length > 0) {
                    for (var i in voucherListArr) {
                        ...
                        if (voucherObj != null) {
                            ...
                            voucherListHtml +=
                                '<div class="form-group" onclick="rechargeVouchaGramWallet(this);" >'
                            voucherListHtml +=
                                '<input type="checkbox" name="voucherChk' +
                                i +
                                '" id="vcrNo' +
                                i +
                                '" >'
                            voucherListHtml +=
                                '<input type="checkbox" name="voucherHdn' +
                                i +
                                '" id="voucherNumberEnc' +
                                i +
                                '" value="' +
                                encryptedVoucherNumber +
                                '" >'
                            voucherListHtml += '<label for="vcrNo">'
                            voucherListHtml += '<div class="vcrPanel">'
                            voucherListHtml +=
                                '<span class="vcrNumber">' + voucherNumber + '</span>'
                            voucherListHtml +=
                                '<div class="vcrAmnt">INR <span id="points">' +
                                value +
                                '</span></div>'
                            voucherListHtml += '</div>'
            ...
            if (resObj != null) {
                ...
                if (responseCode == '000') {
                    ...
                    if (resData != null) {
                        if (resData.CODE == '00') {
                            resMsg =
                                "<p style='color:green; font-size:15px;'>Your voucher has been added successfully!</p>"
...
function vouchaGramResendOtp() {
    ...
    intervalID = setInterval(() => {
        if (TIMER_SECONDS < 0) {
            ...
            document.getElementById('showResendOTP').innerHTML =
                '<input type ="button" value= "Resend OTP" id="showOtpBtn" onclick = "generateVouchaGramRedeemOtp(); vouchaGramResendOtpDisable();  "/>'
            ...
            if (timer_flag) {
                ...
                document.getElementById('resendTimer').innerHTML =
                    'OTP will expire in ' + TIMER_SECONDS + ' seconds'
            }
        }
                    ...
                    if (responseData != null) {
                        ...
                        if (splitFlag == 'N') {
                            ...
                            mainMsgDiv.innerHTML =
                                "<p style='color:green; font-size:18px; text-align:center;'> <b>Congratulations! You have redeemed your GyFTR wallet amount of ₹" +
                                redeemAmt +
                                '</b></p>'
...
function showPointsPeMobileNumberDiv() {
    ...
    if (checkBoxEle.checked) {
        ...
        var orderTotalAmount = document.getElementById('ccsurchargeTotal').value
        document.getElementById('autoDebitBtn').value =
            'Pay ' + currencyCode + ' ' + orderTotalAmount
    }
}
        ...
        if (cardDetails != null) {
            ...
            if (statusCode == '0') {
                if (availableAmount > 250) {
                    ...
                    if (
                        setSelValue == 'CC' &&
                        parseFloat(ppBalanceForCheck) >= parseFloat(orderTotalAmount)
                    ) {
                    ...
                    ppMobNumMsg.style.display = 'block'
                    ppMobNumMsg.innerHTML =
                        'You do not have enough balance, please use another mobile number.'
...
function resetPointsPeChanges() {
    ...
    document.getElementById('ppValueOfPointsDiv').style.visibility = 'hidden'
    document.getElementById('pointsPeMobileNumberDiv').style.display = 'none'
    document.getElementById('yesBankRedeemRewardPoints').value = 'N' //otp div
    ...
    ppVerifyBtn.style.display = 'none'
    var ppSendOtpBtn = document.getElementById('ppSendOtpBtn')
    ppSendOtpBtn.style.display = 'block' //yesbank20230522
            ...
            if (this.response != null) {
                ...
                if (resData != null) {
                    ...
                    if (balance > 0) {
                        ...
                        var acMobNumMsg = document.getElementById('acMobNumMsg')
                        acMobNumMsg.style.display = 'block'
                        document.getElementById('acMobNumMsg').innerHTML =
                            'You do not have enough balance in your AdvantageClub wallet, please use another mobile number.'
                    } else {
                        ADVANTAGE_CLUB_MOBILE_NUMBER = null
                ...
                if (responseCode == '000' && responseMessage == 'SUCCESS') {
                    ...
                    if (responseData != null) {
                        ...
                        if (splitFlag == 'N') {
                            ...
                            mainMsgDiv.style.display = 'block'
                            mainMsgDiv.innerHTML =
                                "<p style='color:green; font-size:18px; text-align:center;'> <b>Congratulations! You have redeemed your Advantage Club wallet amount of ₹" +
                                redeemAmt +
                                '</b></p>'
