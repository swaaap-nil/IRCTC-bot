...
import wait from './models/timer.mjs'
function initLogout() {
    const logoutParams = {
...
//----------------------        33333333       -----------------------------
const { responseBody: trainList,cookies : cookies1 } = await searchTrains(trainSearchParameters);
global.setCookies(cookies1)
await wait(5);

//----------------------        44444444       -----------------------------
const {responseBody: fareAndAvailDetails, cookies : cookies2} = await findFareAndAvail(availibiltySearchParams)
...
//----------------------        10 10 10 10 10 10       -----------------------------
const { responseBody: paymentRedirectHtml, cookies: cookies4 } = 
...
//----------------------        11 11  11  11  11       -----------------------------
const { cookies: cookies5, responseBody: irctcRequestActionResponse } = await irctcRequestAction({
    cookies: global.getCookies(),
    encData: encData
})
